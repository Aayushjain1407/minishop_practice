...
class CartTest < ActiveSupport::TestCase
  ...
  test "calculates total amount" do
    ...
    assert_equal 29.0, @cart.total_amount
  end

  test "adds product to cart" do
    @cart.add(@product1)
    assert_includes @cart.cart_items, 
                    CartItem.find_by(product: @product1)
  end

  test "does not add product to cart if it's already in the cart" do
    @cart.add(@product1)
    @cart.add(@product1)
  ...
  test "removes product from cart" do
    ...
    assert_not_includes @cart.cart_items, 
                    CartItem.find_by(product: @product1)

  end

  test "empties cart" do
    @cart.add(@product1)
    @cart.add(@product2)
    @cart.empty!
